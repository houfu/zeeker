...
class TestUtilityFunctions:
    ...
    def test_download_from_s3_to_dir_success(self, mock_boto3):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mock_boto3.assert_called_with(
                "s3", region_name="us-west-2", endpoint_url=None, config=ANY
            )
    ...
    def test_download_from_s3_to_dir_no_bucket(self, mock_boto3):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            logger.error.assert_called_with("S3_BUCKET environment variable is required")
...
class TestCliCommands:
    ...
    def test_cleanup_command_with_backups(
        self, mock_path_class, mock_datetime, mock_rmtree, mock_setup_logging
    ):
...
class TestErrorHandling:
    ...
    @patch("scripts.manage.boto3.client")
    @patch.dict(os.environ, {"S3_BUCKET": "test-bucket"})
    def test_list_databases_exception_handling(
        self, mock_boto3, mock_load_dotenv, mock_setup_logging
    ):
...
class TestIntegrationScenarios:
    ...
    def test_full_refresh_workflow(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("scripts.manage.Path") as mock_path_class:
                ...
                assert result.exit_code == 0
                assert (
                    "Data changes detected" in result.output
                    or "Downloading fresh data from S3" in result.output
                )
    ...
    def test_status_comprehensive_check(self, mock_setup_logging):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (static_dir / "databases" / "courts" / "custom.css").write_text("court styles")
