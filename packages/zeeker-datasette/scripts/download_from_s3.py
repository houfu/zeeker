...
class ZeekerS3Downloader:
    ...
    def _download_database_files(self) -> Set[str]:
        ...
        try:
            ...
            page_iterator = paginator.paginate(Bucket=self.s3_bucket, Prefix=self.s3_databases_path)
    ...
    def _download_base_assets(self) -> bool:
        ...
        try:
            ...
            self._download_s3_directory(
                f"{self.s3_assets_default_path}/templates/", self.templates_dir
            )

            # Download base static files
            self._download_s3_directory(f"{self.s3_assets_default_path}/static/", self.static_dir)

            # Download base plugins
            self._download_s3_directory(f"{self.s3_assets_default_path}/plugins/", self.plugins_dir)
        ...
        try:
            ...
            if self.templates_dir.exists():
                self._upload_directory_to_s3(
                    self.templates_dir, f"{self.s3_assets_default_path}/templates/"
                )

            # Upload static files
            if self.static_dir.exists():
                self._upload_directory_to_s3(
                    self.static_dir, f"{self.s3_assets_default_path}/static/"
                )

            # Upload plugins
            if self.plugins_dir.exists():
                self._upload_directory_to_s3(
                    self.plugins_dir, f"{self.s3_assets_default_path}/plugins/"
                )
        ...
        try:
            ...
            for db_name in databases:
                ...
                try:
                    ...
                    self.s3_client.download_file(
                        self.s3_bucket, db_metadata_key, str(temp_metadata)
                    )
    ...
    def _download_s3_directory(self, s3_prefix: str, local_dir: Path) -> None:
        ...
        paginator = self.s3_client.get_paginator("list_objects_v2")
        page_iterator = paginator.paginate(Bucket=self.s3_bucket, Prefix=s3_prefix)
