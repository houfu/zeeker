    ...
    try:
        ...
        try:
            response = s3.list_objects_v2(
                Bucket=s3_bucket, Prefix="assets/databases/", Delimiter="/"
            )
            ...
            if uncustomized:
                click.echo(
                    f"   • {len(uncustomized)} database(s) using default assets only: {', '.join(uncustomized)}"
                )
            ...
            if orphaned:
                click.echo(
                    f"   ⚠️  {len(orphaned)} customization(s) without database files: {', '.join(orphaned)}"
                )
        except Exception as e:
